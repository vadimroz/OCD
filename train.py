...
def train(args, config, optimizer, optimizer_scale,
    ...
    if 1:
        ...
        ws,hs,outs = [],[],[]
        with tqdm(train_loader, unit="Generate Data", leave=False, position=0) as tepoch:
        ...
        print('Loading ws, outs and hs\n')
        ws = torch.load('./OCD/checkpoints/ws_new.pt')
        outs = torch.load('./OCD/checkpoints/outs_new.pt')
        hs = torch.load('./OCD/checkpoints/hs_new.pt')
    print('Start Training')
    for epoch in range(epochs):
        print(f'Currently processing epoch {epoch}\n')
        avg_loss = 0
        count = 0
        optimizer.zero_grad()
        difflosslogger = 0
        optimizer_scale.zero_grad()
        with tqdm(train_loader, unit="Batches", leave=True, position=0) as tepoch:
            for idx, batch in enumerate(tepoch):
                optimizer_scale.zero_grad()
                batch['input'] = batch['input'].to(device)
                batch['output'] = batch['output'].to(device)
        ...
        if ((epoch + 1) % n_checkpoint == 0) or (epoch + 1 == epochs):
            ...
            torch.save(ema_helper.state_dict(),checkpoint_path+f'model_checkpoint_epoch{epoch}_step{step}_data{args.datatype}_V3.pt')
            torch.save(scale_model.state_dict(),checkpoint_path+f'scale_model_checkpoint_epoch{epoch}_loss{step}_data{args.datatype}_V3.pt')
